...
const Dashboard = () => {
    ...
    return (
        <main className='bg-slate-600 pb-20 px-3 w-full min-h-screen'>
            <section>
                    ...
                    <div className='w-1/6 text-white flex flex-col gap-5 mt-10'>
                        <NavLink to="/" className={({ isActive }) => (isActive ? 'active' : 'inactive border px-4 hover:bg-slate-500')}>Home</NavLink>
                        {
                            !isAdminLoading && isadmin ? <>
                                ...
                                <NavLink to="/dashboard/addPackage" className={({ isActive }) => (isActive ? 'active' : 'inactive')}>Add Packages</NavLink>
                                <NavLink to="/dashboard/manage-users" className={({ isActive }) => (isActive ? 'active' : 'inactive')}>Manage Users</NavLink>
                            </> :
                                isTourGuide ? <>
                                    <NavLink to="/dashboard" className={({ isActive }) => (isActive ? 'active' : 'inactive')}>My Profile</NavLink>
